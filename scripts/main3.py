...
def master_func(person_class,
    ...
    for solution_index in range(len(solutions_dataframe.columns)):
        # print(f"Assigned People: {[person.name for person in names_to_people.values() if person.assigned_site is not None]}")
        all_people = []
...
if __name__  == "__main__":
    # site_map = pd.read_excel("C:/Users/aditya/Desktop/my_projects/beam/site_leading/github_script/beam_site_coordination/tests/test_f23/edited/v1/empty_site_map.xlsx")
    # person_class = DecalMember

    # site_map = pd.read_excel('C:/Users/aditya/Desktop/my_projects/beam/site_leading/github_script/beam_site_coordination/SiteLeader/4.xlsx')
    # person_class = StaffMember

    # site_map = pd.read_excel('C:/Users/aditya/Desktop/my_projects/beam/site_leading/github_script/beam_site_coordination/StaffMember/4.xlsx')
    site_map = pd.read_excel('C:/Users/aditya/Desktop/my_projects/beam/site_leading/github_script/beam_site_coordination/StaffMember/frozen4.xlsx')
    person_class = DecalMember


    decal_members = pd.read_excel('C:/Users/aditya/Desktop/my_projects/beam/site_leading/github_script/beam_site_coordination/tests/test_f23/edited/v1/decal_members.xlsx', header=[0])
    print(len(decal_members))
    k = 69
    chosen_indices = random.sample(range(len(decal_members)), k)
    decal_members = decal_members.loc[chosen_indices]
    site_leaders = pd.read_excel("C:/Users/aditya/Desktop/my_projects/beam/site_leading/github_script/beam_site_coordination/tests/test_f23/edited/v2/site_leaders.xlsx")
    staff_members = pd.read_excel('C:/Users/aditya/Desktop/my_projects/beam/site_leading/github_script/beam_site_coordination/tests/test_f23/edited/v2/non_SL_staff_members.xlsx')
    time_tolerance = 30

    num_experiments = 20
    initial_read(site_map, site_leaders, staff_members, decal_members,
                 time_tolerance)
    master_func(person_class, num_experiments)
